...
function lin_2020(RGB) {
	...
	const x = 1.09929682680944 ;
	const y = 0.018053968510807;
	...
	return RGB.map(function (val) {
		...
		return sign * (Math.pow((abs + x -1 ) / x, 1/0.45));
	});
}
...
function gam_2020(RGB) {
	...
	return RGB.map(function (val) {
		...
		if (abs > y ) {
			return sign * (x * Math.pow(abs, 0.45) - (x - 1));
		}
...
export { LCH_to_sRGB, sRGB_to_LCH, CSS_color_to_LCH, rgb_array_to_LCH }
